...
public class Fenetre_achat {
    private Fenetre fenetre;

    public Fenetre_achat(Fenetre fenetre) {
        this.fenetre = fenetre;
    }

    public JScrollPane createAchatPanel(int userId) {
        JPanel containerPanel = new JPanel();
        containerPanel.setLayout(new BorderLayout());
        containerPanel.setBackground(new Color(20, 20, 20)); // Fond sombre


        // ** Titre "Mes Achats" avec icônes **
        ...
        // Icône gauche
        JLabel leftIcon = new JLabel(loadIcon("info7/bag.png", 100, 100));
        titlePanel.add(leftIcon);

        // Titre
        JLabel titleLabel = new JLabel("Mes Achats");
        titleLabel.setFont(new Font("Arial", Font.BOLD, 36));
        titleLabel.setForeground(Color.WHITE);
        titlePanel.add(titleLabel);

        // Icône droite
        JLabel rightIcon = new JLabel(loadIcon("info7/bag.png", 100, 100));
        ...
        // Charger les achats de l'utilisateur connecté
        List<String> achat = Database.getAchat(userId);


        if (achat.isEmpty()) {
            JLabel noAchatLabel = new JLabel("Aucun comics en vu.");
            noAchatLabel.setHorizontalAlignment(SwingConstants.CENTER);
            noAchatLabel.setFont(new Font("Arial", Font.BOLD, 20));
            noAchatLabel.setForeground(Color.WHITE);
            mainPanel.setLayout(new BorderLayout());
            mainPanel.add(noAchatLabel, BorderLayout.CENTER);
        } else {
            for (String comicId : achat) {
    ...
    private JPanel createComicPanel(String comicId, String title, String imageUrl) {
        ...
        comicPanel.setBackground(new Color(30, 30, 30)); // Couleur d’arrière-plan
        ...
        comicPanel.addMouseListener(new java.awt.event.MouseAdapter() {
            ...
            public void mouseEntered(java.awt.event.MouseEvent e) {
                comicPanel.setBackground(new Color(50, 50, 50)); // Survol : couleur plus claire
                animateComicPanel(imageLabel, imageUrl, true); // Agrandir l’image
            }

            @Override
            public void mouseExited(java.awt.event.MouseEvent e) {
                comicPanel.setBackground(new Color(30, 30, 30)); // Réinitialiser la couleur
                animateComicPanel(imageLabel, imageUrl, false); // Réduire l’image
            }
